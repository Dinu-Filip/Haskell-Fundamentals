...
getValue i = (snd . lookUp i)

getLocals :: State -> State
getLocals s = [v | v@(i, (scope, val)) <- s, scope == Local]

getGlobals :: State -> State
getGlobals s = [v | v@(i, (scope, val)) <- s, scope == Global]
...
assignArray (A []) (I i) (I v) = A [(i, v)]
assignArray (A (a@(ai, av) : as)) (I i) (I v)
  | ai == i = A ((i, v) : as)
  | otherwise = A (a : as')
    
    where A as' = assignArray (A as) (I i) (I v)

updateVar :: (Id, Value) -> State -> State
updateVar (i, v) [] = [(i, (Local, v))]
updateVar (i, v) (s@(i', (scope, val)) : restS)
  | i == i' = (i, (scope, v)) : restS
  | otherwise = s : updateVar (i, v) restS
...
applyOp Less (I v1) (I v2) = case compare v1 v2 of
  LT -> I 1
  _  -> I 0
applyOp Equal (I v1) (I v2) = case compare v1 v2 of
  EQ -> I 1
  _  -> I 0
applyOp Index (A as) (I v) = I (fromMaybe 0 (lookup v as))

bindArgs :: [Id] -> [Value] -> State
-- Pre: the lists have the same length
bindArgs ids vs = zip ids (map (Local, ) vs)

evalArgs :: [Exp] -> [FunDef] -> State -> [Value]
evalArgs es defs s = map (\e -> eval e defs s) es
...
eval (Const c) _ _         = c
eval (Var v) defs s        = getValue v s
eval (Cond e b1 b2) defs s = case eval e defs s of
  I 1 -> eval b1 defs s
  I 0 -> eval b2 defs s
eval (OpApp op e1 e2) defs s = applyOp op (eval e1 defs s) (eval e2 defs s)
eval (FunApp i es) defs s    = eval e defs (as' ++ s)
  
  where (as, e) = lookUp i defs
        as'     = bindArgs as (evalArgs es defs s)
